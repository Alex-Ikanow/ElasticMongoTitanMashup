...
public class MongoOpLogMonitor implements Runnable {
	...
	@Inject
	protected DatastoreEventFactory _datastoreEventFactory;
		...
		try {
			...
			for (;;) {
				if ((null == dbc) || (0 == dbc.getCursorId())) { // Need to create a new tailable cursor on MongoDB oplog
					...
					dbc = _datastoreService.getOperationsCursor(query);
				}
					...
					try {
								...
								!namespace.startsWith("local.") && !namespace.equals("?"))
						{
							...
							if ('i' == op_type_code) { // Insert "o"
								...
								_datastoreEventFactory.onNewDocument(db_collection, op_obj).async_execute();
							}
								...
								_datastoreEventFactory.onUpdatedDocument(db_collection, op_obj_id, op_update_cmd).async_execute();
							}
								...
								if (op_coll_cmd.containsField("drop")) {
									_datastoreEventFactory.onDeletedCollection(db_collection, op_coll_cmd).async_execute();
								}
							}
							else if ('d' == op_type_code) { // Delete aka Remove obj id'd by "o"
								BasicDBObject op_obj_id = (BasicDBObject) op_dbo.get("o");
								_datastoreEventFactory.onDeletedDocument(db_collection, op_obj_id).async_execute();
							}
